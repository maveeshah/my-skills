...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per the **separate-before-serializing-shared-state** principle (`principles/separate-before-serializing-shared-state.md`).
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per the **encode-lessons-in-structure** principle (`principles/encode-lessons-in-structure.md`).
- Validate at boundaries, trust types inside, per the **boundary-discipline** principle (`principles/boundary-discipline.md`). Business logic as pure functions; the shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per the **make-operations-idempotent** principle (`principles/make-operations-idempotent.md`). Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per the **laziness-protocol** and **minimize-reader-load** principle (`principles/minimize-reader-load.md`)s.
