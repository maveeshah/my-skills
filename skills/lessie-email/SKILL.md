...
description: "Send, manage, and automate emails across multiple providers (Gmail, Outlook, Official Email) via MCP. Use this skill whenever the user wants to send emails, check inbox, manage email threads, create bulk email campaigns, manage drafts, or check email accounts and quotas."
---
...
Add the Lessie Email MCP server to your MCP config (Claude Code `~/.claude.json`, Antigravity`~/.claude.json`, Codex `~/.codex/config.toml`, etc.):
...
- Antigravitypagination: all list endpoints use `cursor` + `size`  --  pass `next_cursor` from previous response.
