...
Before spawning investigators, list the `mcp__*` tools available in this session. That tool list is the dependable source; `~/.claude/.mcp.json` names the configured servers but says nothing about which are reachable right now.
