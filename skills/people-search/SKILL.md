...
description: "Search, qualify, and enrich people and companies. Use this skill whenever the user wants to find professionals, candidates, or KOLs by title, company, location, seniority, or audience; enrich known contacts with email, phone, or LinkedIn; research companies for industry, funding, tech stack, or hiring activity; look up contact info; source candidates for recruiting; generate B2B lead lists; or perform background research on people or organizations."
---
...
Add to your MCP config (Claude Code `~/.claude.json`, Antigravity`~/.claude.json`, Codex `~/.codex/config.toml`, etc.):
