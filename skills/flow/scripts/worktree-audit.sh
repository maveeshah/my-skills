...
# `gh` on PATH is not necessarily the GitHub CLI: an unrelated script of the
# same name is a common shadow, and it would make every PR column silently
# wrong rather than empty. Check before trusting it.
prs=$(mktemp)
echo "[]" > "$prs"
if command -v gh >/dev/null && gh --version 2>&1 | grep -q 'github\.com/cli/cli'; then
	gh pr list --author "@me" --state all --limit 1000 \
		--json number,state,headRefName 2>/dev/null > "$prs" || echo "[]" > "$prs"
else
	echo "warn: no GitHub CLI on PATH; PR column will be empty" >&2
fi

# Transcripts dir: ~/.claude/projects/<slugified-repo-path>/, flat, one
# .jsonl per session. The slug keeps its leading dash, so do NOT strip the
# leading slash: /home/you/proj -> -home-you-proj. Stripping it resolves to a
# directory that does not exist, and LAST_CHAT comes back blank for every row.
