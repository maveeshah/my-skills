...
6. **Verify the chain from transcripts, not self-report.** Read the transcripts at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the workspace path with every "/" turned into "-", leading dash included. Do not glob across `~/.claude/projects/*/`; that crosses workspace boundaries and reads private chats from unrelated projects. A candidate spawned as a subagent has no file of its own: its turns are inlined in the parent's transcript tagged `"isSidechain": true`, so filter on that field rather than looking for one file per candidate. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
