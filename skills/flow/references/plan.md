...
Produce a phased implementation plan grounded in the **Principles** section of the `flow` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `flow` skill end to end, and the leaf files in the **principles** skill it indexes. The principles govern every plan decision; cross-link them.
...
- Browser, Electron, and web UIs: the `claude-in-chrome` skill, or a harness from the **verify** skill.
- CLIs and TUIs: the built-in `/run` skill, or a harness from the **verify** skill.
...
In the overview, name which flow non-negotiables the implementer must apply, by name:

- the **how** skill over each unfamiliar subsystem before changing it.
- the **review** skill for adversarial review on contested designs before shipping.
...
- driving the PR to green after opening it. No bundled playbook covers this; see `parked/README.md` for why.
