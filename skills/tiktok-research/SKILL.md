...
description: "Research high-performing TikTok videos from tracked accounts using Apify TikTok Scraper. Identifies outlier content, analyzes top 5 videos with AI, and generates reports with actionable hook formulas. Triggers: tiktok research, tt research, find trending tiktoks, analyze tiktok accounts, what works on tiktok, content research tiktok, tiktok analysis, tiktok trends."
---
...
```bash
python3 ../video-content-analyzer/scripts/analyze_videos.py \
...
python3 scripts/fetch_tiktok.py -o "$RUN_FOLDER/raw.json" && \
python3 scripts/analyze_posts.py -i "$RUN_FOLDER/raw.json" -o "$RUN_FOLDER/outliers.json" && \
python3 ../video-content-analyzer/scripts/analyze_videos.py -i "$RUN_FOLDER/outliers.json" -o "$RUN_FOLDER/video-analysis.json" -p tiktok
```
