...
Transcripts live at:
- **Claude Code**: `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the workspace path with every "/" turned into "-", leading dash included (so `/home/you/proj` becomes `-home-you-proj`). Derive it from the working directory: `~/.claude/projects/$(pwd | sed 's#/#-#g')`. One file per session, one JSON record per line. Subagent turns are inlined in the same file tagged `"isSidechain": true`. `~/.claude/history.jsonl` holds the user prompts as `{display, project, sessionId, timestamp}` for cheap grepping.
- **Antigravity**: `~/.gemini/antigravity-cli/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` (compact JSONL) or `transcript_full.jsonl` (full log).
