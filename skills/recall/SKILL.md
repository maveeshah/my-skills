...
Transcripts live at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the workspace path with every "/" turned into "-", leading dash included (so `/home/you/proj` becomes `-home-you-proj`). Derive it from the working directory rather than guessing: `~/.claude/projects/$(pwd | sed 's#/#-#g')`. One file per session, one JSON record per line. Subagent turns are inlined in the same file tagged `"isSidechain": true`, not split into separate files, so filter on that field rather than looking for a per-subagent transcript.

`~/.claude/history.jsonl` holds only the user's own prompts, as `{display, project, sessionId, timestamp}`. Grep it first to find candidate sessions cheaply, then open the matching `.jsonl`.
