...
description: "Research high-performing YouTube videos in a niche using TubeLab outlier detection API. Identifies outlier videos, analyzes top relevant videos with AI, and generates reports with actionable hook formulas. Triggers: youtube research, find outlier videos, research YouTube trends, what videos are performing well, find content ideas for my channel, youtube trends."
---
...
```
python scripts/get_channel_videos.py CHANNEL_ID [--format json|summary]
```
...
```
python scripts/find_outliers.py --keywords K1 K2 K3 K4 --adjacent-keywords A1 A2 A3 A4 --output-dir DIR [options]
```
