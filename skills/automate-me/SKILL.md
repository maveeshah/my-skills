...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill (install `skill-creator@claude-plugins-official` if absent, or fall back to the **writing-for-agents** skill in this repo) for authoring, and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
The **flow** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as flow's.
...
- The **flow** skill: example of the output shape.
- The **unslop** skill: prose discipline for every line.
- the `skill-creator` skill (install `skill-creator@claude-plugins-official` if absent, or fall back to the **writing-for-agents** skill in this repo): skill authoring process and writing guidelines.
