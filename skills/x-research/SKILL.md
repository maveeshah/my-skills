---
name: x-research
description: "Research high-performing X/Twitter content from tracked accounts using Apify Tweet Scraper V2. Identifies outlier tweets, trending topics, and content patterns to inform content strategy. Triggers: x research, twitter research, find trending tweets, analyze x accounts, what works on twitter, content research x, tweet analysis."
---
...
python3 scripts/fetch_tweets.py -o "$RUN_FOLDER/raw.json" && \
python3 scripts/analyze_posts.py -i "$RUN_FOLDER/raw.json" -o "$RUN_FOLDER/outliers.json"
```

With video analysis (optional):
```bash
python3 ../video-content-analyzer/scripts/analyze_videos.py -i "$RUN_FOLDER/outliers.json" -o "$RUN_FOLDER/video-analysis.json" -p x
```
